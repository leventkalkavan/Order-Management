...
namespace OrderManagement.Web.Controllers
{
    public class AboutController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var response = await client.GetAsync("http://localhost:5026/api/About");
        ...
        public async Task<IActionResult> CreateAbout(CreateAboutWebDto createAboutWebDto)
        {
            ...
            var response = await client.PostAsync("http://localhost:5026/api/About", stringContent);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"http://localhost:5026/api/About/{id}");
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"http://localhost:5026/api/About/{id}");
        ...
        public async Task<IActionResult> UpdateAbout(UpdateAboutWebDto dto, string id)
        {
            ...
            var response = await client.PutAsync($"http://localhost:5026/api/About/{id}", content);
