...
namespace OrderManagement.Web.Controllers
{
    public class ContactController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var response = await client.GetAsync("http://localhost:5026/api/Contact");
        ...
        public async Task<IActionResult> CreateContact(CreateContactWebDto createContactWebDto)
        {
            ...
            var response = await client.PostAsync("http://localhost:5026/api/Contact", stringContent);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"http://localhost:5026/api/Contact/{id}");
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"http://localhost:5026/api/Contact/{id}");
        ...
        public async Task<IActionResult> UpdateContact(UpdateContactWebDto dto, string id)
        {
            ...
            var response = await client.PutAsync($"http://localhost:5026/api/Contact/{id}", content);
