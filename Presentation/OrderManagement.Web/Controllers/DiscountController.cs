...
namespace OrderManagement.Web.Controllers
{
    public class DiscountController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var response = await client.GetAsync("http://localhost:5026/api/Discount");
        ...
        public async Task<IActionResult> CreateDiscount(CreateDiscountWebDto createDiscountWebDto)
        {
            ...
            var response = await client.PostAsync("http://localhost:5026/api/Discount", stringContent);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.DeleteAsync($"http://localhost:5026/api/Discount/{id}");
            if (response.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var response = await client.GetAsync($"http://localhost:5026/api/Discount/{id}");
        ...
        public async Task<IActionResult> UpdateDiscount(UpdateDiscountWebDto dto, string id)
        {
            ...
            var response = await client.PutAsync($"http://localhost:5026/api/Discount/{id}", content);
